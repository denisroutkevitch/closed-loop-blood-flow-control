...
function [t_set, DR_set, BP_set, Flow_set, CO2_set] = NIRS_average_norm_signals(chal_path, challenges)
    ...
    % load("Flow Baselines.mat");
    load("Pig Flow Baselines.mat");
    % load("Pig Flow Baselines apriori.mat");
    ...
    for chal_num = 1:length(challenges)
        ...
        norm_vel = bl_table(contains({bl_table.Experiment}, chal_name(1:6))).MeanFlow-1;
        
        % set the cutoff frequency for noise removal 
        cutoff_freq = 0.01;  % pig experiments
        % cutoff_freq = 0.05; % rat experiments
        
        % filter the signals
        BP_filt = robust_butter(time, BP, 2,cutoff_freq);
        BP_filt = BP_filt- mean(BP_filt(1:find(time==0)-100));
        Flow_filt = robust_butter(time, Flow, 2,cutoff_freq)-1;
